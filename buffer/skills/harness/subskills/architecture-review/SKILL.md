...
description: Find architecture improvement opportunities: shallow modules, deep module candidates, DDD dependency violations, testability gaps. Use periodically (every few days) to prevent AI-accelerated code rot. Use when codebase feels complex, untestable, or hard to navigate.
---
...
Before exploring, read `CONTEXT.md` and relevant ADRs following the consumer contract in [../engineering/harness-domain-language/DOMAIN-AWARENESS.md](../engineering/harness-domain-language/DOMAIN-AWARENESS.md).
...
- **Naming a deepened module after a concept not in `CONTEXT.md`?** Add the term — same discipline as `harness-domain-language` (see [../engineering/harness-domain-language/CONTEXT_FORMAT.md](../engineering/harness-domain-language/CONTEXT_FORMAT.md)). Create the file lazily if it doesn't exist.
- **Sharpening a fuzzy term?** Update `CONTEXT.md` right there.
- **User rejects with a load-bearing reason?** Offer an ADR: _"Want me to record this so future reviews don't re-suggest it?"_ (see [../engineering/harness-domain-language/ADR_FORMAT.md](../engineering/harness-domain-language/ADR_FORMAT.md)).

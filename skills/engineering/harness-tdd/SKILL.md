---
name: harness-tdd
description: Run a role-isolated TDD workflow: RED (tests only, no implementation), GREEN (implementation only, no test changes), REFACTOR (implementation only), REVIEWER (reports only). Use when implementing features with test-first discipline, when the spec requires TDD, or when the user says "TDD", "red green refactor", "test first", or "role-isolated testing".
---
...
- `../harness-domain-language/DOMAIN-AWARENESS.md`
